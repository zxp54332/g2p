...
class G2p(object):
    def __init__(self):
        ...
        # add missing lexicon
        self.cmu['fbi'] = [['EH1', 'F', 'B', 'IY1', 'AY1']]
        self.cmu['ph.d'] = [['P', 'IY1', 'EY1', 'CH', 'D', 'IY1']]
        self.cmu['nba'] = [['EH1', 'N', 'B', 'IY1', 'EY1']]
        self.cmu['mba'] = [['EH1', 'M', 'B', 'IY1', 'EY1']]
        self.cmu['emba'] = [['IY1', 'EH0', 'M', 'B', 'IY1', 'EY2']]
        self.cmu['AM'] = [['EY1', 'EH1', 'M']]
        self.cmu['PM'] = [['P', 'IY', 'EH1', 'M']]
        self.cmu['eg'] = [['IY1', 'JH', 'IY1']]
        self.cmu['EG'] = [['IY1', 'JH', 'IY1']]
        self.cmu['deductible'] = [['D', 'IH0', 'D', 'AH1', 'K', 'T', 'AH0', 'B', 'AH2', 'L']]
        self.cmu['DEDUCTIBLE'] = [['D', 'IH0', 'D', 'AH1', 'K', 'T', 'AH0', 'B', 'AH2', 'L']]
    ...
    def __call__(self, text, vocabulary=False, pos=None):
        if vocabulary and pos is None:
        ...
        words = [w.replace('tys', 'ties') if w.endswith('tys') else w for w in words]
        if vocabulary:
            if len(words) > 1:
                raise ValueError("not vocabulary input!")
            else:
                tokens = list(zip(words, [pos.upper()]))
        else:
...
#         print(tokens)
        ...
        for word, pos in tokens:
            if word not in ['AM', 'PM'] and re.search("[a-z]", word) is None:
                pron = [word]

            elif word in self.homograph2features:  # Check homograph
                pron1, pron2, pos1 = self.homograph2features[word]
                if word in ['read', 'used']:
                    pron = pron1 if pos == pos1 else pron2
                elif pos.startswith(pos1):
                    pron = pron1
                else:
                    pron = pron2
            elif word in self.cmu:  # lookup CMU dict
                if word in ('dr.', 'prof.', 'id', ):
                    pron = self.cmu[word][1]
                else:
                    pron = self.cmu[word][0]
            else: # predict for oov
                pron = self.predict(word)
            
            # hard fix for "the" followed by a vowel-starting word.
            if len(prons) > 2 and prons[-3:-1] == ['DH', 'AH0'] and any(c in pron[0] for c in 'AEIOU'):
                prons[-2] = "IH1"
                
            prons.extend(pron)
            prons.extend([" "])
        
        return prons[:-1], ' '.join(words)
